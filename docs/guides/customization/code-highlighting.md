...
Called with the block's plain text and the detected language token. Return the highlighted inner HTML, or `undefined` when your highlighter does not know the language — the block then stays plain, keeping its language annotation.
