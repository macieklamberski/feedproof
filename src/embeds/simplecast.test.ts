import { describe, expect, it } from 'bun:test'
import { extractSimplecastEpisode, simplecastResolveEmbed } from './simplecast.js'

const uuid = '7f2c9a10-3b4d-4e5f-8a9b-0c1d2e3f4a5b'

describe('extractSimplecastEpisode', () => {
  it('should read the current player uuid', () => {
    expect(extractSimplecastEpisode(`https://player.simplecast.com/${uuid}?dark=false`)).toEqual({
      id: uuid,
      isCurrent: true,
    })
  })

  it('should read the legacy embed id', () => {
    expect(extractSimplecastEpisode('https://embed.simplecast.com/a1b2c3d4?color=fff')).toEqual({
      id: 'a1b2c3d4',
      isCurrent: false,
    })
  })

  it('should read the legacy numeric form', () => {
    expect(extractSimplecastEpisode('https://simplecast.com/e/1234567?style=medium')).toEqual({
      id: '1234567',
      isCurrent: false,
    })
  })

  it('should return undefined for a simplecast url naming no episode', () => {
    expect(extractSimplecastEpisode('https://simplecast.com/pricing')).toBeUndefined()
  })

  it('should return undefined for a url that cannot be parsed', () => {
    expect(extractSimplecastEpisode('https://[')).toBeUndefined()
  })

  it('should return undefined for a simplecast url naming no path', () => {
    expect(extractSimplecastEpisode('https://simplecast.com/')).toBeUndefined()
  })
})

describe('simplecastResolveEmbed', () => {
  // 200 in 75 of 75 sampled corpus iframes.
  it('should state the fixed player height', () => {
    expect(simplecastResolveEmbed(`https://player.simplecast.com/${uuid}`)).toEqual({
      provider: 'simplecast',
      id: uuid,
      src: `https://player.simplecast.com/${uuid}`,
      height: 200,
    })
  })

  it('should mint the player host from the share host, which names the same uuid', () => {
    expect(simplecastResolveEmbed(`https://play.simplecast.com/${uuid}`)).toEqual({
      provider: 'simplecast',
      id: uuid,
      src: `https://player.simplecast.com/${uuid}`,
      height: 200,
    })
  })

  // The legacy id is a separate id space: the server maps `fc9a4d22` to a uuid we cannot
  // compute, so the url stands and the iframe follows the redirect itself.
  it('should keep a legacy embed url rather than speak its id to the player host', () => {
    expect(simplecastResolveEmbed('https://embed.simplecast.com/a1b2c3d4')).toEqual({
      provider: 'simplecast',
      id: 'a1b2c3d4',
      src: 'https://embed.simplecast.com/a1b2c3d4',
      height: 200,
    })
  })

  it('should keep a legacy numeric url', () => {
    expect(simplecastResolveEmbed('https://simplecast.com/e/1234567')).toEqual({
      provider: 'simplecast',
      id: '1234567',
      src: 'https://simplecast.com/e/1234567',
      height: 200,
    })
  })

  it('should return undefined for a simplecast url naming no episode', () => {
    expect(simplecastResolveEmbed('https://simplecast.com/pricing')).toBeUndefined()
  })
})
