...
export const defaultNonContentSelectors = [
  ...
  '.tmblr-alt-text-helper', // Tumblr alt-text badge holding the literal label "ALT" beside an image: 311 feeds (0.002%), 1,935 occurrences. On Tumblr the badge reveals the image's alt text on click. The feed already carries that text in the image's own alt attribute, so the badge is redundant chrome that renders as a stray "ALT". The image is a preceding sibling, never a descendant, so removing the badge leaves it and its alt intact.
