...
import { extractAmpCache } from './ampCache.js'

describe('extractAmpCache', () => {
  it('should extract HTTPS target from canonical host', () => {
    const url = new URL('https://cdn.ampproject.org/c/s/example.com/article')

    expect(extractAmpCache(url)).toBe('https://example.com/article')
  })
  ...
  it('should extract HTTPS target from publisher subdomain', () => {
    ...
    expect(extractAmpCache(url)).toBe('https://www.bbc.com/news/amp/business-48879976')
  })
  ...
  it('should extract HTTP target when /s/ prefix is missing', () => {
    ...
    expect(extractAmpCache(url)).toBe('http://www.bbc.co.uk/news/amp/technology-40932487')
  })

  it('should return null for non-/c/ paths on the same host', () => {
