...
import { extractMimecast } from './mimecast.js'

describe('extractMimecast', () => {
  it('should extract target from url param', () => {
    const url = new URL(
      'https://protect-us.mimecast.com/s/abc123?url=https%3A%2F%2Fexample.com%2Farticle&token=xyz',
    )

    expect(extractMimecast(url)).toBe('https://example.com/article')
  })
  ...
  it('should synthesise https URL from domain param', () => {
    ...
    expect(extractMimecast(url)).toBe('https://example.com')
  })
  ...
  it('should match other regional Mimecast subdomains', () => {
    ...
    expect(extractMimecast(url)).toBe('https://example.com/page')
  })
