...
import { extractYahooSearchRedirect } from './yahooSearch.js'

describe('extractYahooSearchRedirect', () => {
  it('should extract target from RU path segment', () => {
    ...
    expect(extractYahooSearchRedirect(url)).toBe('https://example.com/article')
  })

  it('should return null for paths without RU= segment', () => {
    const url = new URL('https://r.search.yahoo.com/search?p=test')

    expect(extractYahooSearchRedirect(url)).toBeUndefined()
  })
