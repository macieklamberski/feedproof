import { describe, expect, it } from 'bun:test'
import { extractSmartredirect } from './smartredirect.js'

describe('extractSmartredirect', () => {
  it('should extract target from url param', () => {
    const url = new URL('https://smartredirect.de/?url=https%3A%2F%2Fexample.com%2Fproduct')

    expect(extractSmartredirect(url)).toBe('https://example.com/product')
  })

  it('should return null when url param is missing', () => {
    const url = new URL('https://smartredirect.de/?other=value')

    expect(extractSmartredirect(url)).toBeUndefined()
  })

  it('should return null for non-smartredirect hosts', () => {
    const url = new URL('https://example.com/?url=https%3A%2F%2Fother.com')

    expect(extractSmartredirect(url)).toBeUndefined()
  })
})
