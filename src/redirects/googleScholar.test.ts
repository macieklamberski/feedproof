...
import { extractGoogleScholarRedirect } from './googleScholar.js'

describe('extractGoogleScholarRedirect', () => {
  it('should extract target from url param', () => {
    const url = new URL(
      'https://scholar.google.com/scholar_url?url=https%3A%2F%2Fcite.com%2Fpaper.pdf&hl=en',
    )

    expect(extractGoogleScholarRedirect(url)).toBe('https://cite.com/paper.pdf')
  })
  ...
  it('should return null when url param is missing', () => {
    ...
    expect(extractGoogleScholarRedirect(url)).toBeUndefined()
  })
