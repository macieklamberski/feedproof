...
// Matches hrefs that point at an image file (the lightbox / full-size link),
// not a post permalink.
...
const findFullUrl = (image: Element): string | undefined => {
  ...
  const href = parent.getAttribute('href') ?? undefined

  return href && imageUrlRegex.test(href) ? href : undefined
}

// Walks up to the image's enclosing item <figure> (stopping at the gallery
// itself) and returns its direct <figcaption>, the per-image caption.
...
export const extractGalleryItems = (
  ...
  for (const image of gallery.querySelectorAll(imageSelector)) {
    // Skip <noscript> fallback images (the pipeline's fixLazyImages normally
    // removes these first; this keeps the resolver correct when used standalone).
