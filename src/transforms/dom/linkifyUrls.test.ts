...
describe('linkifyUrls', () => {
  ...
  it('should link bare https URL', async () => {
    const result = await transform('<p>Visit https://example.com for more</p>')
  ...
  it('should link bare http URL', async () => {
    expect(await transform('<p>Visit http://example.com for more</p>')).toContain(
  ...
  it('should link URL with path and query', async () => {
    expect(
      await transform('<p>See https://example.com/path?key=value&other=1#hash for details</p>'),
    ).toContain('href="https://example.com/path?key=value&other=1#hash"')
  })

  it('should link multiple URLs in one paragraph', async () => {
    const result = await transform('<p>See https://one.com and https://two.com</p>')
  ...
  it('should link URLs across multiple paragraphs', async () => {
    const result = await transform('<p>See https://one.com</p><p>And https://two.com</p>')
  ...
  it('should preserve surrounding text when linkifying', async () => {
    const result = await transform('<p>before https://example.com after</p>')
  ...
  it('should link URL in deeply nested content', async () => {
    expect(
      await transform('<div><section><p><em>See https://deep.com here</em></p></section></div>'),
    ).toContain('href="https://deep.com"')
  })

  it('should handle URL at start of text node', async () => {
    expect(await transform('<p>https://example.com is a URL</p>')).toContain(
      '<a href="https://example.com"',
    )
  })

  it('should handle URL at end of text node', async () => {
    expect(await transform('<p>Visit https://example.com</p>')).toContain(
      'href="https://example.com"',
    )
  })

  it('should not double-link already linked URL', async () => {
    const result = await transform('<p><a href="https://example.com">https://example.com</a></p>')

    expect(result.match(/<a /g)).toHaveLength(1)
  })

  it('should not link protocol-less URL', async () => {
    expect(await transform('<p>Visit example.com for more</p>')).not.toContain('<a')
  })

  it('should not link email address', async () => {
    expect(await transform('<p>Contact user@example.com</p>')).not.toContain('<a')
  })

  for (const tag of ['pre', 'code', 'kbd', 'samp', 'var']) {
    it(`should not link URL inside ${tag} tag`, async () => {
      expect(await transform(`<${tag}>https://example.com</${tag}>`)).not.toContain('<a')
    })
  }

  it('should not link URL inside nested code within pre', async () => {
    expect(
      await transform('<pre><code>const url = "https://example.com"</code></pre>'),
    ).not.toContain('<a')
  })

  it('should not modify html with no URLs', async () => {
    expect(await transform('<p>No links here</p>')).toContain('<p>No links here</p>')
  })

  it('should handle whitespace-only text nodes', async () => {
    expect(await transform('<p>   </p>')).not.toContain('<a')
  })

  it('should handle empty content', async () => {
    expect(await transform('')).not.toContain('<a')
  })
})
