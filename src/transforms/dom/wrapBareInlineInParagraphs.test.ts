...
describeForEachParser('wrapBareInlineInParagraphs', (parseHtml) => {
  ...
  describe('dissolving wrappers', () => {
    ...
    it('should wrap inside section, article, header, footer, main', async () => {
      expect(await transform('<section>text</section>')).toEqualHtml(
        '<section><p>text</p></section>',
      )
      expect(await transform('<article>text</article>')).toEqualHtml(
        '<article><p>text</p></article>',
      )
      expect(await transform('<header>text</header>')).toEqualHtml('<header><p>text</p></header>')
      expect(await transform('<footer>text</footer>')).toEqualHtml('<footer><p>text</p></footer>')
      expect(await transform('<main>text</main>')).toEqualHtml('<main><p>text</p></main>')
    })
  ...
  describe('skipped contexts', () => {
    ...
    it('should not wrap inside pre or code', async () => {
      expect(await transform('<pre><div>code</div></pre>')).toEqualHtml(
        '<pre><div>code</div></pre>',
      )
      expect(await transform('<pre><code><div>x</div></code></pre>')).toEqualHtml(
