...
describe('hljsHighlightFn', () => {
  it.each(Object.keys(extraLanguages))('should resolve the %s grammar', (language) => {
    expect(hljsHighlightFn('x', language)).toBeDefined()
  })

  it.each(
    Object.entries(languageAliases).flatMap(([language, aliases]) =>
      aliases.map((alias): [string, string] => [alias, language]),
    ),
  )('should resolve the %s alias to %s', (alias) => {
