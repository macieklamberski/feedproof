...
| `canonicalizeAlignment` | Canonicalize explicit media alignment (WordPress `align*` classes, bare `center`/`left`/`right` classes on the media element, deprecated `align` attribute, `<center>`, inline `text-align`, image auto-margins) into a single `data-align="center\|left\|right"` hook on the image/video/iframe or its `<figure>`. Text alignment, `alignnone`, and vertical `align` are left alone |
